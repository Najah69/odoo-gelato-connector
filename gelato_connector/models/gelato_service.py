    ...
    def _post(self, path, payload):
        ...
        try:
            ...
            try:
                ...
                _logger.error('Gelato POST %s — non-JSON response (status %s): %s',
                              url, resp.status_code, resp.text[:200])
                raise UserError(f"Gelato returned an invalid (non-JSON) response: {exc}")
    ...
    def _get(self, path):
        ...
        try:
            ...
            try:
                ...
                _logger.error('Gelato GET %s — non-JSON response (status %s): %s',
                              url, resp.status_code, resp.text[:200])
                raise UserError(f"Gelato returned an invalid (non-JSON) response: {exc}")
        except requests.exceptions.HTTPError as exc:
            body = exc.response.text[:500]
