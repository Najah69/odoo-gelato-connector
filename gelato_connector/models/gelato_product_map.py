...
class GelatoProductMap(models.Model):
    ...
    * Or query the Gelato Products API:
    ...
    product_type = fields.Selection(
        [('poster', 'Poster'), ('frame', 'Framed Print'), ('dibond', 'Dibond / Alu'),
         ('canvas', 'Canvas'), ('photo_book', 'Photo Book'), ('mug', 'Mug'), ('other', 'Other')],
        string='Product Type', required=True, default='poster',
    )
    gelato_product_uid = fields.Char(
        string='Gelato productUid', required=True,
    ...
    shipment_method_uid = fields.Selection(
        ...
        string='Shipment Method', default='standard', required=True,
    )
    price = fields.Float(
        string='Sale Price', digits=(10, 2),
    ...
    odoo_product_id = fields.Many2one(
        'product.template', string='Linked Odoo Product',
        domain=[('type', '=', 'service')],
        ondelete='set null',
